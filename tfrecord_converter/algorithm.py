...
def assign_point_cloud_to_bounding_boxes(
    ...
    :param bounding_boxes_tfs: [N x 4 x 4]
           Homogeneous transforms in point cloud coordinate system
    :param bounding_boxes_dims: [N x 3] Bounding box dimensions (length, width, height)
    :return: Tuple: number of points within each bounding box,
             mapping from point cloud to bounding box ID
    ...
    for i, (box, dims) in enumerate(zip(bounding_boxes_tfs, bounding_boxes_dims)):
        ...
        pc = np.concatenate(
            (
                point_cloud[:, :3],
                np.ones(shape=(point_cloud.shape[0], 1), dtype=point_cloud.dtype),
            ),
            axis=-1,
        )
